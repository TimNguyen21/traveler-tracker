...
let domUpdates = {
  ...
  populateTravelerInfo(userID, travelersData) {
    ...
    let presentTripsSummary =
      this.populateUserTrips(traveler.filterPresentTrips());
  ...
  populateUserTrips(tripsData) {
    return tripsData.reduce((tripsSummary, trip) => {
      ...
      `<section class="trip-card">
        <img class="destination-image" src="${trip.destinationImage}"
          alt="${trip.destinationAlt}">
        ...
        <div><span>Overall Trip Cost:</span>
        ${formatter.format(trip.overallCost)}</div>
      </section>`
    }, '')
  },
  ...
  openForm() {
    return $(".search-destination-image").click(function() {
      ...
      <input id="enter-request-date"
        class="enter-request-date date${currentDestinationID}"
          type='text' placeholder="YYYY/MM/DD"></input>
      <label for="enter-request-travelers">Enter Travelers:</label>
      <input id="enter-request-travelers"
        class="enter-request-travelers travelers${currentDestinationID}"
          type="number"></input>
      <label for="enter-request-duration">Enter Duration:</label>
      <input id="enter-request-duration"
        class="enter-request-duration duration${currentDestinationID}"
          type="number"></input>
      <button data-id="${currentDestinationID}"id="calculate-request-cost"
        class="calculate-request-cost">Calculate Total Cost</button>
      <label for="estimate-total-cost">Total Cost + Fees:</label>
      <div id="estimate-total-cost">$0</div>
      <button data-id="${currentDestinationID}"id="submit-trip-request"
        class="submit-trip-request">Submit Request</button>
      </div>
      `);
      ...
      $('.calculate-request-cost').click(function() {
        ...
        $("#estimate-total-cost")
        .html(`${formatter.format(trip.calculateTripTotal($lodgingPerDay,
          $costPerPerson, $travalers, $duration))}`)
      })
      ...
      $(".submit-trip-request").click(function() {
        ...
        let $travalers = $(`.travelers${currentDestinationID}`).val();
        let $duration = $(`.duration${currentDestinationID}`).val();
        trip.submitTripRequest(currentTraveler,
          currentDestinationID, $travalers, $date, $duration);
        $(event.target).closest('.trip-request-form')
          .html(`Your Request is Processed`)
      })
    })
  },
  ...
  populateAgencyInfo(agencyId, agency) {
    ...
    let pendingRequestSummary = pendingRequest.reduce((summary, request) => {
      ...
      `<section class="pending-summary">
        ...
        <div>Name: ${agency.travelData
          .find(traveler => traveler.id === request.userID).name}</div>
        <div>Destination: ${agency.destinationData
          .find(destination => destination.id === request.destinationID)
            .destination}</div>
        <div>Duration: ${request.duration} days</div>
        <div>Status: ${request.status}</div>
        <button data-id="${request.id}" class="approve-button">
          Approve Request</button>
        <button data-id="${request.id}" class="deny-button">
          Deny Request</button>
      </section>`
    }, '')

    let usersCurrentlySummary =
      usersCurrentlyOnTrip.reduce((summary, request) => {
      return summary +=
      `<section class="pending-summary">
  ...
  travelerSearchResults(userSearchResult) {
    return userSearchResult.reduce((summary, request) => {
      ...
      `<section class="search-summary">
        ...
        <div>Duration: ${request.duration} days</div>
        <div>Status: ${request.status}</div>
        <div>Total Cost + Agency Fees:
          ${formatter.format(request.totalCostPlusAgentFees)}</div>
        <button data-id="${request.tripID}" class="search-deny-button">
